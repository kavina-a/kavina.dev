...
function Scene({
  ...
  useFrame((_, delta) => {
    ...
    const targetSpeed = 0.22 + (1 - Math.pow(curOutRef.current - 1, 2)) * 1.15
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      ...
      uInverted: new THREE.Uniform(1), // kavina.me is always dark aesthetic
...
bool drawOval(vec2 polarUv, vec2 polarCenter, float a, float b, bool reverseGradient, float softness, out vec4 color) {
    ...
    if (edge > 0.0) {
        ...
        color = vec4(vec3(gradient), 0.92 * edge);
        return true;
    }
    return false;
}
...
void main() {
    ...
    // Swirl the ovals harder as volume rises
    float noise = flow(decomposed, radius * 0.03 - uAnimation * 0.2) - 0.5;
    theta += noise * mix(0.28, 0.72, uOutputVolume);
    ...
    for (int i = 0; i < 7; i++) {
        ...
        a = 0.42 + noise * 0.5;
        b = noise * mix(4.4, 2.6, uInputVolume);
    ...
    float inputRadius1 = radius + uInputVolume * 0.38;
    float inputRadius2 = radius + uInputVolume * 0.28;
    float opacity1 = mix(0.28, 0.78, uInputVolume);
    float opacity2 = mix(0.2, 0.58, uInputVolume);
